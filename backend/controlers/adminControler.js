...
const addDoctor = async (req, res) => {
    try {
        const { name, email, password, speciality, degree, experience, about, fees, address, mapLocation } = req.body;
        const imageFile = req.file;

        // Checking for all data to add doctor
        if (!name || !email || !password || !speciality || !degree || !experience || !about || !fees || !address || !mapLocation) {
...
// API to get All dotors list for Admin panel
...
//API to get All appointement 
const appointementsAdmin = async (req, res) => {
    try {
        const appointements = await appointementModel.find({})
...
// API for appointement cancellation
